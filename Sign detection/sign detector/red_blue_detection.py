...
def blue_color_select(img):
    ...
    for cnt in cnts:
         x_, y_, weight, height = cv2.boundingRect(cnt)
         # if x_ < 0.5 * w_img:   # traffic signs are always on the right hand
         #    continue
         if y_ > 0.5 * h_img:
             continue
         #contour area and perimeter
...
def main(img):
    ...
    cnts_circle = blue_color_select(img)
    cnt_combine = cnts_circle + cnts_all
    for cnt in cnt_combine:
        x, y, w, h = cv2.boundingRect(cnt)
...
if __name__=="__main__":
    # test image
    import os
    # img = cv2.imread('E:\ex_python\sign_detection\CODE\\images\\20201216_131256.jpg')
    ...
    #
    # video = VideoFileClip('E:\ex_python\sign_detection\\videos\\Img 2150 Autobahn Rightlane Somesigns.m4v')
    # video_after_process = video.fl_image(main)
    # video_after_process.write_videofile("E:\ex_python\sign_detection\\videos\\Img 2150 Autobahn Rightlane Somesigns_proecess.mp4", audio=False)  #