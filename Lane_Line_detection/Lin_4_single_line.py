...
def make_coordinate(image, line_parameters): # make coordinate system for left and right lines
    slope, intercept = line_parameters
    print(image.shape) # height ,width, channel
    y1 = image.shape[0] # The bottom of y 704
...
def average_slope_intercept(image, lines): # Combine two lines into single line
    ...
    for line in lines:
        ...
        if slope <0:
            left_fit.append((slope, intercept)) # left
        else:
            right_fit.append((slope,intercept)) # right
...
def display_lines(image, lines):  # Display lines in black image
    ...
    if lines is not None:
        for line in lines:
            #print(line)
            x1, y1, x2, y2 = line.reshape(4)
            cv2.line(line_image, (x1, y1), (x2, y2), (255, 0, 0), 3) #draw line
    return line_image
...
line_image = display_lines(lane_image, average_line) # Modified
combo = cv2.addWeighted(lane_image, 0.8, line_image , 1 ,1) #blend the original image and lines
#cv2.imshow("Line_imge", line_image )
cv2.imshow("Mix", line_image )
cv2.waitKey(0)